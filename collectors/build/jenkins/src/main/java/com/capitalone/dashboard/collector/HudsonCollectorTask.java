...
public class HudsonCollectorTask extends CollectorTask<HudsonCollector> {
	...
	// LogFactory.getLog(HudsonCollectorTask.class);
	...
	@Autowired
	public HudsonCollectorTask(TaskScheduler taskScheduler, HudsonCollectorRepository hudsonCollectorRepository,
			HudsonJobRepository hudsonJobRepository, BuildRepository buildRepository, HudsonClient hudsonClient,
			HudsonSettings hudsonSettings, ComponentRepository dbComponentRepository) {
		super(taskScheduler, "TeamCityBuild");
	...
	public void collect(HudsonCollector collector) {
		...
		for (String instanceUrl : collector.getBuildServers()) {
			...
			try {
				...
				activeServers.remove(instanceUrl); // since it was a rest
													// exception, we will not
													// delete this job and wait
													// for
	...
	private void clean(HudsonCollector collector, List<HudsonJob> existingJobs) {
		...
		for (HudsonJob job : existingJobs) {
			if ((job.isEnabled() && !uniqueIDs.contains(job.getId())) || (!job.isEnabled() && uniqueIDs.contains(job.getId()))) {
	 ...
	 * @param collector
	 */
	private void deleteUnwantedJobs(List<HudsonJob> activeJobs, List<HudsonJob> existingJobs, List<String> activeServers,
			HudsonCollector collector) {

		List<HudsonJob> deleteJobList = new ArrayList<>();
		for (HudsonJob job : existingJobs) {
			if (job.isPushed())
				continue; // build servers that push jobs will not be in active
							// servers list by design

			// if we have a collector item for the job in repository but it's
			// build server is not what we collect, remove it.
			if (!collector.getBuildServers().contains(job.getInstanceUrl())) {
				deleteJobList.add(job);
			}

			// if the collector id of the collector item for the job in the repo
			// does not match with the collector ID, delete it.
			if (!job.getCollectorId().equals(collector.getId())) {
				deleteJobList.add(job);
			}

			// this is to handle jobs that have been deleted from build servers.
			// Will get 404 if we don't delete them.
			log(job.getInstanceUrl());
			log(job.toString());
		...
		for (HudsonJob job : enabledJobs) {
			...
			for (Build buildSummary : nullSafe(buildsByJob.get(job))) {
				if (isNewBuild(job, buildSummary)) {
					buildSummary.setCollectorItemId(job.getId());
					buildRepository.save(buildSummary);
					count++;
				}else{
					log("Not a new build : "+buildSummary.getNumber());
				}
			}
		}
	...
	private void addNewJobs(Set<HudsonJob> jobs, List<HudsonJob> existingJobs, HudsonCollector collector) {
		log("Exisiting jobs empty ? "+CollectionUtils.isEmpty(existingJobs));
		long start = System.currentTimeMillis();
		int count = 0;
	...
	private List<HudsonJob> enabledJobs(HudsonCollector collector, String instanceUrl) {
		return hudsonJobRepository.findEnabledJobs(collector.getId(), instanceUrl);
	}

	@SuppressWarnings("unused")
	private HudsonJob getExistingJob(HudsonCollector collector, HudsonJob job) {
		return hudsonJobRepository.findJob(collector.getId(), job.getInstanceUrl(), job.getJobName());
	}
